...
const knowledgeBase = [
    ...
    },
    {
        ...
        response: 'I speak Japanese and English, so feel free to ask questions in either—though I respond here in English.'
    },
    {
    ...
    return (
        <div className='chat-app'>
            <header className='chat-header'>
                <h1>{chatTitle.title}</h1>
                <p>{chatTitle.tagline}</p>
            </header>

            <section className='chat-log' aria-live='polite'>
                {messages.map((message, index) => (
                    <div key={`${message.id}-${index}`} className={`chat-message ${message.sender}`}>
